...
// Calm raised card: soft border + lift on hover.
const RAISED =
  'rounded-2xl border border-line bg-paper shadow-soft ' +
  'transition duration-calm hover:-translate-y-0.5 hover:shadow-lift';

// Inner well for visuals inside a raised card.
const WELL = 'rounded-xl border border-line bg-mist/70';
    ...
    <svg viewBox={`0 0 ${w} ${h}`} aria-hidden="true" className="h-full w-full overflow-visible">
      <defs>
        <linearGradient id="grow" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor="#5C6B52" stopOpacity="0.35" />
          <stop offset="100%" stopColor="#5C6B52" stopOpacity="0" />
        </linearGradient>
      </defs>
      <polygon points={area} fill="url(#grow)" />
      <polyline points={line} fill="none" stroke="#5C6B52" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
      <circle cx={w} cy={h - (pts[pts.length - 1] / max) * h} r="3.5" fill="#7A8A6E" />
    </svg>
  );
}
...
function PositioningMock() {
  ...
  return (
    <svg viewBox="0 0 120 84" aria-hidden="true" className="h-full w-full">
      <line x1="10" y1="74" x2="114" y2="74" stroke="rgba(28,25,23,0.12)" strokeWidth="1" />
      <line x1="10" y1="6" x2="10" y2="74" stroke="rgba(28,25,23,0.12)" strokeWidth="1" />
      {dots.map((d, i) => (
        <circle key={i} cx={d.x} cy={d.y} r={d.r} fill="rgba(168,162,158,0.55)" />
      ))}
      <circle cx="34" cy="26" r="7" fill="#5C6B52" />
      <circle cx="34" cy="26" r="11" fill="none" stroke="#7A8A6E" strokeWidth="1.5" opacity="0.7" />
    </svg>
  );
}
    ...
    <div className="min-h-screen overflow-x-hidden bg-canvas text-ink">
      ...
      <header className="sticky top-0 z-40 border-b border-line bg-canvas/90 backdrop-blur-xl">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-5 py-4">
          ...
          <nav className="hidden items-center gap-7 text-sm text-ink-soft md:flex">
            <a href="#features" className="transition duration-calm hover:text-ink">Features</a>
            <a href="#how" className="transition duration-calm hover:text-ink">How it works</a>
            <a href="#why" className="transition duration-calm hover:text-ink">Why Mira</a>
          </nav>
          <div className="flex items-center gap-2">
            {authed ? (
              <Link href="/app" className="inline-flex h-11 items-center rounded-xl bg-accent px-4 text-sm font-semibold text-white transition duration-calm hover:bg-accent-dim">Go to app</Link>
            ) : (
              <>
                <Link href="/login" className="inline-flex h-11 items-center rounded-xl px-3 text-sm font-medium text-ink-soft transition duration-calm hover:text-ink">Sign in</Link>
                <Link href="/signup" className="inline-flex h-11 items-center rounded-xl bg-accent px-4 text-sm font-semibold text-white transition duration-calm hover:bg-accent-dim">Create account</Link>
              </>
            )}
          </div>
        </div>
      ...
      <section className="relative isolate overflow-hidden">
        ...
        <div className="pointer-events-none absolute inset-0 -z-10" aria-hidden="true">
          ...
          />
          <motion.div
            className="absolute right-[14%] top-[8%] h-[360px] w-[360px] rounded-full bg-accent/15 blur-[130px]"
          ...
          />
          <motion.div
            className="absolute left-1/2 top-[18%] h-[280px] w-[280px] -translate-x-1/2 rounded-full bg-mist blur-[120px]"
          ...
          <div
            ...
            style={{
              background:
                'conic-gradient(from 180deg at 50% 50%, transparent 0deg, rgba(92,107,82,0.10) 90deg, transparent 180deg, rgba(92,107,82,0.08) 270deg, transparent 360deg)',
          ...
          <div
            ...
            style={{
              backgroundImage:
                'linear-gradient(to right, rgba(92,107,82,0.08) 1px, transparent 1px), linear-gradient(to bottom, rgba(92,107,82,0.08) 1px, transparent 1px)',
        ...
        <div className="mx-auto max-w-4xl px-5 pt-20 pb-14 text-center md:pt-28">
          <motion.a
            ...
            className="inline-flex items-center gap-2 rounded-full border border-line bg-mist px-3 py-1 text-xs font-medium text-ink-soft backdrop-blur transition hover:border-line-strong"
          >
            <span className="flex h-1.5 w-1.5 rounded-full bg-emerald-400" />
            Built around your business
            <Icon name="chevronRight" className="h-3 w-3 text-ink-soft" />
          </motion.a>
          ...
          <motion.h1
            ...
            className="mt-6 font-brand text-4xl font-semibold leading-[1.08] tracking-tight text-ink sm:text-6xl md:text-7xl"
          >
          ...
          >
            {authed ? (
              <div className="flex flex-col items-center justify-center gap-3 sm:flex-row">
                <Link href="/app" className="inline-flex items-center gap-1.5 inline-flex h-12 items-center rounded-xl bg-accent px-6 text-base font-semibold text-white shadow-soft transition hover:bg-accent-dim">
                  <Icon name="sparkle" className="h-4 w-4" /> Go to app
                </Link>
                <a href="#how" className="inline-flex h-12 items-center rounded-xl border border-line bg-paper px-6 text-base font-medium text-ink transition duration-calm hover:bg-mist">See how it works</a>
              </div>
            ) : (
              <div className="flex flex-col items-center justify-center gap-3 sm:flex-row">
                <Link href="/signup" className="group inline-flex items-center gap-1.5 inline-flex h-12 items-center rounded-xl bg-accent px-6 text-base font-semibold text-white shadow-soft transition hover:bg-accent-dim">
                  Create your account <Icon name="chevronRight" className="h-4 w-4 transition group-hover:translate-x-0.5" />
                </Link>
                <Link href="/login" className="inline-flex h-12 items-center rounded-xl border border-line bg-paper px-6 text-base font-medium text-ink transition duration-calm hover:bg-mist">Sign in</Link>
              </div>
            )}
          </motion.div>
        </div>
        ...
        <div className="mx-auto max-w-5xl px-5 pb-20">
          ...
          >
            ...
            <div className="overflow-hidden rounded-2xl border border-line bg-ink-900 shadow-lift">
              <div className="flex items-center gap-1.5 border-b border-line bg-mist/60 px-4 py-3">
                <span className="h-2.5 w-2.5 rounded-full bg-rose-500/60" />
                <span className="h-2.5 w-2.5 rounded-full bg-amber-500/60" />
                <span className="h-2.5 w-2.5 rounded-full bg-emerald-500/60" />
                <span className="ml-3 text-xs text-ink-soft">Mira · Business report · Acme Analytics</span>
              </div>
              <div className="grid gap-4 p-5 md:grid-cols-3">
                {/* Market growth tile */}
                <div className="rounded-xl border border-line bg-mist/60 p-4 text-left">
                  <div className="flex items-center justify-between">
                    <p className="text-xs text-ink-soft">Market size</p>
                    <span className="chip border-emerald-200 bg-emerald-50 text-emerald-700">+18% CAGR</span>
                  </div>
                  <p className="mt-1 text-2xl font-bold text-ink">$4.2B</p>
                  <div className="mt-2 h-[60px]"><GrowthMock /></div>
                </div>
                {/* Positioning tile */}
                <div className="rounded-xl border border-line bg-mist/60 p-4 text-left">
                  <p className="text-xs text-ink-soft">Where you stand</p>
                  <div className="mt-2 h-[88px]"><PositioningMock /></div>
                  <p className="mt-1 text-xs text-accent">You: best-value quadrant</p>
                </div>
                ...
                <div className="flex flex-col gap-3">
                  {[
                    ...
                    <div key={k.label} className="flex items-center gap-3 rounded-xl border border-line bg-mist/60 p-3">
                      <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-accent/10 text-accent">
                        <Icon name={k.icon} className="h-4 w-4" />
                      </span>
                      <div>
                        <p className="text-base font-semibold leading-none text-ink">{k.value}</p>
                        <p className="mt-1 text-xs text-ink-soft">{k.label}</p>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="px-5 pb-5">
                <div className="rounded-xl border border-accent/20 bg-accent/5 p-4 text-left">
                  <p className="text-xs font-semibold uppercase tracking-wide text-accent">Analyst take</p>
        ...
        <div className="mx-auto grid max-w-5xl grid-cols-2 gap-px px-5 md:grid-cols-4">
          {STATS.map((s) => (
            <div key={s.label} className="px-4 py-8 text-center">
              ...
              <p className="mt-1.5 text-xs text-ink-soft">{s.label}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section id="features" className="mx-auto max-w-6xl px-5 py-24">
        <Reveal className="mx-auto max-w-2xl text-center">
          ...
          <h2 className="mt-3 text-3xl font-bold tracking-tight text-ink md:text-5xl">Everything you need to understand your business</h2>
          <p className="mt-4 text-ink-soft">Tailored to your business. It replaces weeks of research and guesswork.</p>
        </Reveal>
          ...
          {SPOTLIGHT.map((f, i) => (
            <Reveal key={f.title} delay={i * 0.08}>
              <div className={`group h-full overflow-hidden p-6 ${RAISED}`}>
                <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-accent/15 text-accent ring-1 ring-accent/20">
          ...
          {FEATURES.map((f, i) => (
            <Reveal key={f.title} delay={(i % 4) * 0.06}>
              <div className={`group h-full p-5 ${RAISED}`}>
                <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-mist text-accent transition group-hover:bg-accent/15">
      ...
      {/* How it works */}
      <section id="how" className="border-y border-line bg-mist/40">
        <div className="mx-auto max-w-5xl px-5 py-24">
          <Reveal className="mx-auto max-w-2xl text-center">
            <span className="text-xs font-semibold uppercase tracking-widest text-accent">How it works</span>
            <h2 className="mt-3 text-3xl font-bold tracking-tight text-ink md:text-5xl">Full report in three steps</h2>
            <p className="mt-4 text-ink-soft">No setup. Just answers.</p>
          </Reveal>
          <div className="relative mt-14 grid gap-8 md:grid-cols-3">
            ...
            {STEPS.map((s, i) => (
              <Reveal key={s.title} delay={i * 0.1} className="relative text-center md:text-left">
                <div className="mx-auto flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-br from-accent to-violet-500 text-sm font-bold text-ink shadow-[0_6px_18px_-4px_rgba(99,102,241,0.7)] md:mx-0">
                  {i + 1}
                </div>
      ...
      <section id="why" className="mx-auto max-w-6xl px-5 py-24">
        <div className="grid gap-12 lg:grid-cols-2 lg:items-center">
          <Reveal>
            <span className="text-xs font-semibold uppercase tracking-widest text-accent">Why Mira</span>
            <h2 className="mt-3 text-3xl font-bold tracking-tight text-ink md:text-5xl">Get clarity before you scale</h2>
      ...
      <section className="px-5 pb-24">
        <Reveal className="relative mx-auto max-w-5xl overflow-hidden rounded-[32px] border border-line bg-gradient-to-b from-accent/15 to-ink-900 px-6 py-20 text-center">
          ...
          <span className="chip border-emerald-200 bg-emerald-50 text-emerald-700">Open to everyone</span>
          <h2 className="mt-5 text-3xl font-bold tracking-tight text-ink md:text-5xl">See your market clearly</h2>
          <p className="mx-auto mt-4 max-w-md text-ink-soft">Create your account and start turning market signals into focused next moves.</p>
          <div className="mt-9">
            {authed ? (
              <Link href="/app" className="inline-flex items-center gap-1.5 rounded-full bg-accent px-7 py-3 text-base font-semibold text-white shadow-soft transition hover:bg-accent-dim">
                <Icon name="sparkle" className="h-4 w-4" /> Go to app
              </Link>
            ) : (
              <Link href="/signup" className="group inline-flex items-center gap-1.5 rounded-full bg-accent px-7 py-3 text-base font-semibold text-white shadow-soft transition hover:bg-accent-dim">
                Create your account <Icon name="chevronRight" className="h-4 w-4 transition group-hover:translate-x-0.5" />
              </Link>
            )}
          </div>
      ...
      <footer className="border-t border-line">
        <div className="mx-auto flex max-w-6xl flex-col items-center justify-between gap-4 px-5 py-8 text-sm text-ink-soft sm:flex-row">
          ...
          <div className="flex items-center gap-6">
            <a href="#features" className="transition hover:text-ink-soft">Features</a>
            <a href="#how" className="transition hover:text-ink-soft">How it works</a>
            <a href="#why" className="transition hover:text-ink-soft">Why Mira</a>
            <Link href="/requests" className="transition hover:text-ink-soft">Feature requests</Link>
            {!authed && <Link href="/login" className="transition hover:text-ink-soft">Sign in</Link>}
            {!authed && <Link href="/signup" className="transition hover:text-ink-soft">Create account</Link>}
          </div>
          <span className="text-xs text-ink-faint">© {new Date().getFullYear()} Mira</span>
        </div>
      </footer>
    </div>
  );
...
const SPOTLIGHT = [
  {
    ...
    visual: (
      <p className="text-xs italic leading-relaxed text-ink-soft">"A mid-market analytics tool for ops teams, sold as a self-serve subscription."</p>
    ),
  },
  {
    icon: 'activity',
  ...
  },
  {
    ...
    visual: (
      <p className="text-xs italic leading-relaxed text-ink-soft">"Own the ops-team customer and launch a self-serve $39 tier."</p>
    ),
  },
];
