...
export default function CompetitorDetailClient({ id }) {
  ...
  const COLORS = ['#5C6B52', '#34d399', '#f59e0b', '#f87171', '#7A8A6E'];

  return (
    <div className="mx-auto w-full max-w-4xl space-y-6">
      <Link href="/competitors" className="inline-flex items-center gap-1 text-sm text-ink-soft hover:text-ink transition">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="flex items-start gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <h1 className="truncate text-xl font-semibold text-ink sm:text-2xl">{competitor.name}</h1>
            <a href={competitor.pricing_url} target="_blank" rel="noreferrer"
              className="mt-1 inline-flex max-w-full items-center gap-1 break-all text-sm text-accent hover:underline">
      ...
      {competitor.last_error && (
        <div className="flex items-start gap-2 rounded-xl border border-rose-200 bg-rose-950/20 px-4 py-3 text-sm text-rose-700">
          <Icon name="alert" className="mt-0.5 h-4 w-4 shrink-0" />
          <div>
            <div className="font-medium">Last fetch failed</div>
            <div className="mt-0.5 text-rose-700/80 text-xs">{competitor.last_error}</div>
          </div>
        </div>
      )}
        ...
        <div className="space-y-4">
          <div className="grid gap-3 sm:grid-cols-3">
            <div className="card p-4 text-center space-y-1">
              <p className="text-xs font-medium uppercase tracking-wide text-ink-soft">Value Score</p>
              <p className="text-2xl font-bold"><ValueScore score={competitor.value_score} /></p>
              <button
                onClick={runValueScore}
                disabled={scoring}
                className="text-xs text-accent hover:text-ink transition"
              >
                {scoring ? 'Scoring…' : competitor.value_score ? 'Re-score' : 'Run AI score'}
              </button>
            </div>
            <div className="card p-4 text-center space-y-1">
              <p className="text-xs font-medium uppercase tracking-wide text-ink-soft">Changes</p>
              <p className="text-2xl font-bold text-ink">{parsedChanges.length}</p>
              <p className="text-xs text-ink-faint">pricing events detected</p>
            </div>
            <div className="card p-4 text-center space-y-1">
              <p className="text-xs font-medium uppercase tracking-wide text-ink-soft">Snapshots</p>
              <p className="text-2xl font-bold text-ink">{data.snapshots?.length || 0}</p>
              <p className="text-xs text-ink-faint">content captures</p>
            </div>
          </div>

          {competitor.value_analysis && (
            <div className="card p-4">
              <h3 className="mb-2 text-xs font-semibold uppercase tracking-wide text-ink-soft">Value Analysis</h3>
              <p className="text-sm text-ink-soft leading-relaxed">{competitor.value_analysis}</p>
            </div>
          )}

          {competitor.description && (
            <div className="card p-4">
              <h3 className="mb-2 text-xs font-semibold uppercase tracking-wide text-ink-soft">Description</h3>
              <p className="text-sm text-ink-soft leading-relaxed">{competitor.description}</p>
            </div>
          )}

          {parsedChanges.length > 0 && (
            <div className="card p-4 space-y-3">
              <h3 className="text-xs font-semibold uppercase tracking-wide text-ink-soft">Recent Changes</h3>
              {parsedChanges.slice(0, 3).map((c) => (
                <div key={c.id} className="flex items-start gap-2">
                  <ImpactBadge impact={c.analysis?.impact} />
                  <div className="flex-1 min-w-0">
                    <p className="text-sm text-ink-soft">{c.analysis?.summary || 'Pricing updated'}</p>
                    <p className="text-xs text-ink-faint">{timeAgo(c.detected_at)}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="card p-4 space-y-4">
          ...
          {priceHistory === null ? (
            <div className="h-64 flex items-center justify-center">
              <Icon name="refresh" className="h-5 w-5 animate-spin text-ink-soft" />
            </div>
          ) : chartData.length < 2 ? (
          ...
          <div className="space-y-2">
            <p className="text-xs text-ink-soft flex items-center gap-2">
              ...
              {latestSnapshot.source === 'apify' && (
                <span className="chip border-ink-700 bg-ink-850 text-ink-soft text-[10px]">via Apify</span>
              )}
            </p>
            <pre className="card max-h-[600px] overflow-auto whitespace-pre-wrap p-5 font-mono text-xs leading-relaxed text-ink-soft">
        ...
        <div className="space-y-4">
          {!battlecard && (
            <div className="card p-5 text-center space-y-3">
              ...
              <h2 className="font-semibold text-ink">Generate Battlecard</h2>
              ...
              {!latestSnapshot && <p className="text-xs text-ink-faint">Refresh to capture a snapshot first.</p>}
            </div>
          )}
            ...
            <div className="space-y-4">
              {battlecard.elevator_pitch && (
                <div className="rounded-xl border border-accent/30 bg-accent/5 p-4">
                  <p className="text-sm font-medium text-accent">"{battlecard.elevator_pitch}"</p>
                </div>
              )}
                    ...
                    <ul className="space-y-1">
                      {items.map((item, i) => (
                        <li key={i} className={`flex items-start gap-2 text-sm ${
                          color === 'rose' ? 'text-rose-700' : color === 'emerald' ? 'text-emerald-700' : 'text-ink-soft'
                        }`}>
              ...
              {battlecard.common_objections?.length > 0 && (
                <div className="card p-4 space-y-3">
                  <h3 className="text-xs font-semibold uppercase tracking-wide text-ink-soft">Common Objections</h3>
                  {battlecard.common_objections.map((obj, i) => (
                    <div key={i} className="space-y-1">
                      <p className="text-xs font-medium text-ink-soft">"{obj.objection}"</p>
                      <p className="text-sm text-ink-soft pl-2 border-l border-accent/30">{obj.response}</p>
                    </div>
                  ))}
                </div>
              )}
    ...
    <div className="card p-4 space-y-2">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-ink-soft">{timeAgo(change.detected_at)}</span>
        </div>
        {change.diff && (
          <button onClick={() => setShowDiff((s) => !s)} className="text-xs text-accent hover:text-ink transition">
            {showDiff ? 'Hide diff' : 'View diff'}
          </button>
        )}
      </div>
      {a.summary && <p className="text-sm text-ink">{a.summary}</p>}
      {a.details && <p className="text-sm text-ink-soft">{a.details}</p>}
        ...
        <pre className="mt-2 max-h-80 overflow-auto rounded-lg border border-ink-800 bg-ink-950 p-3 font-mono text-[11px] leading-relaxed">
          {change.diff.split('\n').map((line, i) => (
            <div key={i} className={
              ...
              line.startsWith('-') && !line.startsWith('---') ? 'text-rose-700' :
              line.startsWith('@@') ? 'text-accent' : 'text-ink-soft'
            }>{line || ' '}</div>
          ))}
        </pre>
      )}
